...
int main(int argc, char *argv[]) {
    ...
    sipiopt.add_option("-r,--region", optRegion, "Select region of interest, where x y w h are integer values.")->expected(4);

    int optReduce = 0;
    sipiopt.add_option("-R,--reduce", optReduce, "Reduce image size by factor  (cannot be used together with --size and --scale)");

    std::string optSize;
    sipiopt.add_option("-s,--size", optSize, "Resize image to given size (cannot be used together with --reduce and --scale)");

    int optScale;
    sipiopt.add_option("-S,--scale", optScale, "Resize image by the given percentage Value (cannot be used together with --size and --reduce)");
    ...
    int optKeepAlive = 5;
    sipiopt.add_option("--keepalive", optKeepAlive, "Number of seconds for the keeop-alive optioon of HTTP 1.1.")->envname("SIPI_KEEPALIVE");
    ...
    sipiopt.add_option("--maxpost", optMaxPostSize, "A string indicating the maximal size of a POST request, e.g. '300M'.")->envname("SIPI_MAXPOSTSIZE");

    std::string optImgroot = "./images";
    sipiopt.add_option("--imgroot", optImgroot, "Root directory containing the images for the web server.")->envname("SIPI_IMGROOT")->check(CLI::ExistingDirectory);

    std::string optDocroot = "./server";
    sipiopt.add_option("--docroot", optDocroot, "Path to document root for normal webserver.")->envname("SIPI_DOCROOT")->check(CLI::ExistingDirectory);
    ...
    sipiopt.add_option("--scriptdir", optScriptDir, "Path to directory containing Lua scripts to implement routes.")->envname("SIPI_SCRIPTDIR")->check(CLI::ExistingDirectory);
    ...
    sipiopt.add_option("--subdirlevels", optSubdirLevels, "Number of subdir levels (deprecated).")->envname("SIPI_SUBDIRLEVELS");

    std::vector<std::string> optSubdirExcludes = { "tmp", "thumb"};
    sipiopt.add_option("--subdirexcludes", optSubdirExcludes, "Directories not included in subdir calculations.")->envname("SIPI_SUBDIREXCLUDES");

    std::string optInitscript = "./config/sipi.init.lua";
    sipiopt.add_option("--initscript", optInitscript, "Path to init script (Lua).")->envname("SIPI_INITSCRIPT")->check(CLI::ExistingFile);
    ...
    sipiopt.add_option("--thumbsize", optThumbSize, "Size of the thumbnails (to be used within Lua).")->envname("SIPI_THUMBSIZE");
    ...
    sipiopt.add_option("--jwtkey", optJWTKey, "The secret for generating JWT's (JSON Web Tokens) (exactly 42 characters).")->envname("SIPI_JWTKEY");
    ...
    sipiopt.add_option("--loglevel", optLogLevel, "Logging level Value can be: 'DEBUG', 'INFO', 'WARNING', 'ERR', 'CRIT', 'ALERT', 'EMERG'.")
    ...
    if (!sipiopt.get_option("--query")->empty()) {
        ...
        if (!sipiopt.get_option("--format")->empty()) {
            switch(optFormat) {
                ...
                case OptFormat::pdf: format = "png"; break;
            }
        }
        else {
            //
        ...
        if (!sipiopt.get_option("--icc")->empty()) {
            Sipi::SipiIcc icc;
            switch(optIcc) {
                case OptIcc::sRGB: icc = Sipi::SipiIcc(Sipi::icc_sRGB); break;
                case OptIcc::AdobeRGB: icc = Sipi::SipiIcc(Sipi::icc_AdobeRGB); break;
                case OptIcc::GRAY: icc = Sipi::SipiIcc(Sipi::icc_GRAY_D50); break;
        ...
        try {
            ...
            switch (optLogLevel) {
                ...
                case LogLevel::NOTICE:  loglevelstring = "NOTICE"; break;
                case LogLevel::WARNING: loglevelstring = "WARNING"; break;
                case LogLevel::ERR:     loglevelstring = "ERR"; break;
                case LogLevel::CRIT:    loglevelstring = "CRIT"; break;
                case LogLevel::ALERT:   loglevelstring = "ALERT"; break;
                case LogLevel::EMERG:   loglevelstring = "EMERG"; break;
            }
            if (!config_loaded) {
